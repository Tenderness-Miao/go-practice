...
func (app *App) monitor() {
	app.signals = make(chan os.Signal, 2)

	switch runtime.GOOS {
	case "windows":
		signal.Notify(app.signals, os.Interrupt, os.Kill, syscall.SIGKILL, syscall.SIGHUP, syscall.SIGINT, syscall.SIGQUIT, syscall.SIGILL, syscall.SIGABRT, syscall.SIGTERM)
	case "linux":
		signal.Notify(app.signals, os.Interrupt, os.Kill, syscall.SIGKILL, syscall.SIGSTOP, syscall.SIGHUP, syscall.SIGINT, syscall.SIGQUIT, syscall.SIGILL, syscall.SIGABRT, syscall.SIGFPE, syscall.SIGSEGV, syscall.SIGTERM)
	case "darwin":
		signal.Notify(app.signals, os.Interrupt, os.Kill, syscall.SIGKILL, syscall.SIGSTOP, syscall.SIGHUP, syscall.SIGINT, syscall.SIGQUIT, syscall.SIGILL, syscall.SIGABRT, syscall.SIGSYS, syscall.SIGTERM)
	}

}

func (app *App) forceShutdown(ctx context.Context) {
	select {
	case <-app.signals:
		log.Println("强制退出")
